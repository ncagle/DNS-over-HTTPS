...
| jitender| https://jit.ddns.net:2443/doh | :heavy_check_mark: | DoH server - India, Oracle Cloud, Hyderabad, India, Runs with nginx, high availability, load balanced by nginx with multiple backend DNS servers. Backend servers run pihole (with unbound recursive resolver) for ad, analytics, trackers blocking provides a clean browsing experience. Does not forward to other public recursive resolvers @coolquasar |
